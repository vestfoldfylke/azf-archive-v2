...
const validateTemplateData = require("./validate-templatedata.js");
...
const addAttachments = (archivePayload, attachments) => {
  ...
  if (!validServices.includes(archivePayload.service))
    throw new Error(`Adding attachment is only allowed in services: '${validServices.toString()}' this template is using service: '${archivePayload.service}'. Why are you doing this??`);
  if (!validMethods.includes(archivePayload.method))
    throw new Error(`Adding attachment is only allowed in methods: '${validMethods.toString()}' this template is using method: '${archivePayload.method}'. Why are you doing this??`);
...
const addContacts = (archivePayload, contacts) => {
  ...
  if (!validServices.includes(archivePayload.service))
    throw new Error(`Adding contacts is only allowed in services: '${validServices.toString()}' this template is using service: '${archivePayload.service}'. Why are you doing this?? :(`);
  if (!validMethods.includes(archivePayload.method))
    throw new Error(`Adding contacts is only allowed in methods: '${validMethods.toString()}' this template is using method: '${archivePayload.method}'. Why are you doing this?? :(`);
