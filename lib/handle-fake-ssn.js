...
const handleFakeSsn = async (birthdate, gender, name, context) => {
  ...
  while (!foundUnique) {
    ...
    logger("info", [`Check for existing PrivatePersons on fake ssn ${fakeSsn}`], context);
    ...
    if (privatePersonRes.length === 1 && privatePersonRes[0].LastName === lastName) {
      logger("info", [`Found existing PrivatePersons on fake ssn ${fakeSsn} with same lastName as provided in body, will use it`], context);
      foundUnique = true;
      resultFakeSsn = fakeSsn;
    } else if (privatePersonRes.length === 0) {
      logger("info", [`No PrivatePerson found on fake ssn ${fakeSsn}, will use it to create new PrivatePerson`], context);
      foundUnique = true;
      resultFakeSsn = fakeSsn;
      ...
      logger("info", [`PrivatePerson found on fake ssn ${fakeSsn}, but not matching lastNames, will generate new fakeSsn and continue checking.`], context);
      runningNumber -= 1;
