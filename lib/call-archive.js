...
module.exports = async (archiveData, context) => {
  ...
  if (isGet && Array.isArray(result)) {
    ...
    while (!finished) {
      logger('info', `More boring stuff here, fetching page ${page} (zero-indexed) of ${totalPages}`, context)
      parameter.Page = page
      /** @type { { data: SIFResponse } } */
      const pageResult = await axios.post(url, { parameter }, { ...httpOptions, headers })
      logger('info', ['Got response', 'service', service, 'method', method, 'page', page], context)
      ...
      if (finished) {
        ...
        logger('info', `Got all ${page} (zero-indexed) of ${totalPages} pages. Let us pray and continue`, context)
      }
    }
