...
const { krr } = require('../krr')

/*
Vurder om vi alltid skal sjekke mot FREG også, i og med at vi ikke har freg-synken i 360 enda.
Når vi har en gyldig person (funnet fnr i FREG), kan vi også slå opp i KRR for å hente epost og mobilnummer. Så legger vi det på privatpersonen i tillegg i 360. Kan sjekke om det faktisk er annerledes enn det som ligger der fra før, og oppdatere kun hvis det er forskjell.

I mitt hode, begynner med å IKKE legge på epost og telefon dersom varslsingsstatus er KAN_IKKE_VARSLES
...
const privatePersonIsUpToDate = (privatePersonToUpdate, privatePersonData) => {
  ...
  if (privatePersonData.email && privatePersonData.email !== privatePersonToUpdate.Email) return false
  if (privatePersonData.phoneNumber && privatePersonData.phoneNumber !== privatePersonToUpdate.PhoneNumber) return false
  return true
}
...
const repackBirthdate = (birthdate) => {
  // YYYY-MM-DD to DDMMYY and D+4DMMYY
  const dateList = birthdate.split('-')
  if (dateList.length !== 3) throw new Error('Birthdate was not on format YYYY-MM-DD, wtf?')
  const year = dateList[0].substring(2, 4)
  const month = dateList[1]
  const day = dateList[2]
  const add4toDay = `${Number(day.substring(0, 1)) + 4}${day.substring(1, 2)}` // To also be able to find fake ssn
  return {
...
const syncPrivatePerson = async (syncPrivatePersonData, context) => {
  ...
  if (fakeSsn || manualData) {
    ...
    if ([name, streetAddress, zipCode, zipPlace].some(ele => typeof ele !== 'string')) throw new HTTPError(400, 'Parameters "name", "streetAddress", "zipCode" and "zipPlace" must be of type string')
    if (email && typeof email !== 'string') throw new HTTPError(400, 'Parameter "email" must be of type string')
    if (phoneNumber && typeof phoneNumber !== 'string') throw new HTTPError(400, 'Parameter "phoneNumber" must be of type string')
  }
  ...
  if (syncPrivatePersonMethod === 'ssn') {
    ...
    if (!forceUpdate) {
      logger('info', ['Identifier is "name and birthdate", forceUpdate is false - we try to get privatePerson directly from P360'], context)
      ...
      const birthdateMatches = namePrivatePersonRes.filter(privatePerson => privatePerson.PersonalIdNumber && (privatePerson.PersonalIdNumber.substring(0, 6) === repackedBirthdate.regular || privatePerson.PersonalIdNumber.substring(0, 6) === repackedBirthdate.fakeSsn)) // If match on name and birthdate as well
      if (birthdateMatches.length === 1) {
        logger('info', ['Identifier is "name and birthdate" found match directly in P360, will use ssn found in P360 as identifier for PrivatePerson'], context)
        privatePersonRes = birthdateMatches
        ssnToUse = birthdateMatches[0].PersonalIdNumber
      } else {
        logger('info', ['Identifier is "name and birthdate" and could not find match directly in P360, will have to try freg'], context)
      }
    }
    if (!ssnToUse) {
  ...
  if (privatePersonRes.length === 0) { // No match - need to create
    ...
    if (manualData || fakeSsn) { // CREATE with manual data
      ...
      const krrData = await krr(ssnToUse)
    ...
    if (forceUpdate) { // Her skal det oppdateres samma hva!
      ...
      const addressProtection = Boolean(foundPrivatePerson.PrivateAddress.StreetAddress && foundPrivatePerson.PrivateAddress.StreetAddress.toLowerCase().includes('sperret'))
  ...
  for (const [key, value] of Object.entries(privatePerson)) {
    if (!['email', 'phoneNumber'].includes(key) && value === null) throw new HTTPError(500, `Oh no, ${key} has null value, developer has made a mistake, tell you know who to fix it...`, privatePerson)
  }
...
module.exports = { syncPrivatePerson, repackFregAddress, getSyncPrivatePersonMethod, getName, repackBirthdate }
